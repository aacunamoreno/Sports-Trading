...
async def calculate_records_from_start_date(start_date: str = "2025-12-22"):
    ...
    # Public Record threshold: only consider games where consensus >= 61%
    PUBLIC_CONSENSUS_THRESHOLD = 61
    ...
    for date in dates:
        ...
        if doc and doc.get('games'):
            ...
            # Calculate Public Record for this date (consensus >= 61% threshold)
            date_public_hits = 0
    ...
    for date in dates:
        ...
        if doc and doc.get('games'):
            ...
            # Calculate Public Record for NCAAB (consensus >= 61% threshold)
            date_public_hits = 0
            date_public_misses = 0
            for game in doc['games']:
                away_pct = game.get('away_consensus_pct') or 0
...
async def update_records_from_start_date(start_date: str = "2025-12-22"):
    ...
    # Update public_records (Public Consensus Record) - uses 61%+ threshold with Covers.com spread
    for league in ["NBA", "NHL", "NCAAB"]:
        ...
        while i < len(lines):
            ...
            if slash_format_match:
                ...
                if over_under_match:
                    ...
                    # Look for risk amount in nearby lines (format: "2,000.00 / 1,818.18")
                    risk_amount = 0
                    win_amount = 0
                    for j in range(i, min(len(lines), i+5)):
                        risk_match = re.search(r'(\d{1,3}(?:,\d{3})*\.?\d*)\s*/\s*(\d{1,3}(?:,\d{3})*\.?\d*)', lines[j])
                        if risk_match:
                            risk_amount = float(risk_match.group(1).replace(',', ''))
                            win_amount = float(risk_match.group(2).replace(',', ''))
                            break
                    ...
                    logger.info(f"[History] Found slash-format bet: {bet_type} {bet_line} ({away_team} vs {home_team}) risk=${risk_amount}")
                    i += 1
                    continue
            ...
            if total_match:
                ...
                if teams_match:
                    ...
                    # Look for risk amount in nearby lines
                    risk_amount = 0
                    win_amount = 0
                    for j in range(i, min(len(lines), i+5)):
                        risk_match = re.search(r'(\d{1,3}(?:,\d{3})*\.?\d*)\s*/\s*(\d{1,3}(?:,\d{3})*\.?\d*)', lines[j])
                    ...
                    logger.info(f"[History] Found TOTAL bet: {bet_type} {bet_line} ({away_team} vs {home_team}) risk=${risk_amount}")
                ...
                if teams_match:
                    ...
                    logger.info(f"[History] Found o/u TOTAL bet: {bet_type} {bet_line} ({away_team} vs {home_team}) risk=${risk_amount}")
            ...
            if spread_match and 'TOTAL' not in line.upper():
                ...
                logger.info(f"[History] Found SPREAD bet: {bet_type} ({away_team} vs {home_team}) risk=${risk_amount}")
            ...
            if current_edge is None or current_edge == 0 or current_edge == '':
                ...
                if combined_value and combined_value > 0:
                    ...
                    line_for_edge = (
                        ...
                        game.get('live_line') or 
                        game.get('opening_line') or 
                        game.get('total')
                    )
...
async def get_public_records_detail(league: str):
    ...
    Uses 61%+ consensus threshold and Covers.com spread data.
    """
    try:
        league_upper = league.upper()
        if league_upper not in ['NBA', 'NHL', 'NCAAB', 'NFL']:
...
async def scrape_first_period_bets_enano():
    ...
    try:
        ...
        for idx, ticket in enumerate(tickets):
            ...
            if not date_match:
                logger.warning(f"[1st Period] Ticket {idx}: No date found")
                continue
        ...
        logger.info(f"[1st Period Bets] === FINAL ===")
